...
parser <- ArgumentParser(description='Signac rLSI Embedding')
...
parser$add_argument('--kWeight', metavar='kWeight', type='integer', default=100, help='k.weight for IntegrateEmbeddings; default 100')
...
cat("Arguments\n")
outFile <- paste(sep="",outPrefix,"_rLSI_args.txt")
...
print_time('Split object by sample')
obj.list <- SplitObject(obj, split.by = sample_col)
obj.list


print_time("Processing object list")
for(idx in names(obj.list)){
    obj.list[[idx]] <- FindTopFeatures(obj.list[[idx]])
    obj.list[[idx]] <- RunTFIDF(obj.list[[idx]])
    obj.list[[idx]] <- RunSVD(obj.list[[idx]])
    
    print(obj.list[[idx]])
}


print_time('Combine object list')
obj.combined <- obj.list[[1]]
for (x in 2:length(obj.list)){ 
    obj.combined <- merge(obj.combined, obj.list[[x]])
}
obj.combined


print_time('Join Layers')
obj.combined <- JoinLayers(obj.combined)
obj.combined


print_time('Combined processing')
obj.combined <- FindTopFeatures(obj.combined)
obj.combined <- RunTFIDF(obj.combined)
obj.combined <- RunSVD(obj.combined)
obj.combined


print_time("Integration anchors")
if(NOrmDim1){
    start_dim=1
} else {
    start_dim=2
...
integration.anchors <- FindIntegrationAnchors(
  ...
  reduction = "rlsi",
  dims = start_dim:max_dim
)
saveRDS(integration.anchors,paste(sep='',outPrefix,'_rLSI_integrationAnchors.rds'))


print_time("Integrate LSI embeddings")
integrated <- IntegrateEmbeddings(
  anchorset = integration.anchors,
  reductions = obj.combined[["lsi"]],
  new.reduction.name = "integrated_lsi",
  dims.to.integrate = 1:max_dim, k.weight = kWeight
)
integrated


print_time("Saving")
emb <- Embeddings(integrated, reduction = "integrated_lsi")
emb <- emb[,start_dim:max_dim]
dim(emb)
emb[1:3,1:3]
saveRDS(emb,paste(sep='',outPrefix,'_rLSI_embeddings.rds'))

saveRDS(integrated,paste(sep='',outPrefix,'_rLSI_object.rds'))

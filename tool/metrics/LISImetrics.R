...
parser <- ArgumentParser(description='LISI-based metrics')
parser$add_argument('embed_file', metavar='embed_file', help='snATAC-seq embedding file')
...
if(!all(file.exists(embed_file,meta_file))) stop("Input file(s) don't exist.")
...
cat("Arguments\n")
outFile <- paste(sep="",outPrefix,"_LISImetrics_args.txt")
